...
csvPath = "L36 -/Game_Shop_Sales_300_Rows.csv"
...
class Main(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.mainTabFrame: ctk.CTkFrame = self.tabView.tab("View")

        self.mainTable = virtualizedTable(self.mainTabFrame, columns=[
                ("date", "Date", 25),
                ("gameTitle", "Game Title", 25),
                ("category", "Category", 25),
                ("unitSold", "Units Sold", 25),
                ("price", "Price (£)", 25),
                ("totalRevenue", "Total Revenue (£)", 25)
            ],
        )
    ...
    def load(self, dataFrame: pl.DataFrame):
        ...
        for row in dataFrame.to_dict():
            for index, _ in enumerate(dataFrame.to_dict()[row]):
                items.append(
                    {
                        "date": str(dataFrame.to_dict()["Date"][index] or ""),
                        "gameTitle": str(dataFrame.to_dict()["Game Title"][index] or ""),
                        "category": str(dataFrame.to_dict()["Category"][index] or ""),
                        "unitSold": str(dataFrame.to_dict()["Units Sold"][index] or ""),
